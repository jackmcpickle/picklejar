...
export function createPicklejar(configPath?: string) {
  ...
  const { coordinator, discoveryAgents, implementationAgents } = createCoordinator()
  ...
  const mastra = new Mastra({
    ...
    storage: new LibSQLStore({
      ...
      url: config.memory.storage === 'libsql' ? 'file:.mastra/picklejar.db' : ':memory:',
    }),
    server: {
  ...
  return { mastra, config, coordinator, discoveryAgents, implementationAgents }
}
