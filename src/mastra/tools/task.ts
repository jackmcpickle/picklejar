...
export const createSubtask = createTool({
    id: 'create-subtask',
    description:
        'Create a subtask under a parent task for delegation to an agent',
    inputSchema: z.object({
        parentId: z.string().describe('Parent task ID'),
        title: z.string().describe('Short task title'),
        description: z.string().describe('Detailed task description'),
        type: z
            .enum(['discovery', 'implementation'])
            .describe('Task type determines which agent pool handles it'),
    }),
    ...
    execute: async ({ parentId, title, description, type }) => {
        const task = taskManager.createSubtask(
            parentId,
            title,
            type,
            description,
...
export const reportCompletion = createTool({
    ...
    description:
        'Report that a task is complete with a summary of what was accomplished',
    inputSchema: z.object({
