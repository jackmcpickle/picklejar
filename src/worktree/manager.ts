...
export async function createWorktree(agentId: string, taskId: string): Promise<WorktreeInfo> {
...
export async function mergeWorktree(taskId: string, targetBranch: string = 'main'): Promise<string> {
  const info = activeWorktrees.get(taskId)
  if (!info) throw new Error(`No worktree for task ${taskId}`)

  const result = await execa('git', ['merge', info.branch, '--no-ff', '-m', `Merge ${info.branch}`], {
    cwd: process.cwd(),
    reject: false,
  })
