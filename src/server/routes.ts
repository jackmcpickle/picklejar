...
export const taskCreateRoute = registerApiRoute('/tasks', {
  ...
  handler: async (c: Context) => {
    ...
    const mastra = c.get('mastra' as never) as { getAgent: (id: string) => { generate: (prompt: string, opts: unknown) => Promise<unknown> } | undefined } | undefined
    ...
    if (coordinator) {
      coordinator.generate(
        ...
        console.error(`Coordinator error on task ${task.id}:`, message)
    ...
    return new Response(
      new ReadableStream({
        start(controller) {
          ...
          controller.enqueue(
            encoder.encode(`data: ${JSON.stringify({ event: 'init', task })}\n\n`),
          )

          const unsubscribe = taskManager.onTaskEvent((taskId, event, updatedTask) => {
